...
fun NewConnectionScreen(
    ...
    BluePilotBackground {
        Column(
            ...
            ControlModesCard(onNavigateToScreen = onNavigateToScreen)
...
@Composable
private fun ControlModesCard(onNavigateToScreen: (String) -> Unit) {
    GlassCard(modifier = Modifier.fillMaxWidth()) {
        Text("Control Modes", color = OnSurface, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
        Text("Open any working HID controller mode.", color = OnSurfaceVariant, style = MaterialTheme.typography.bodySmall)
        Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            GhostButton("Mouse", { onNavigateToScreen("mouse_keyboard") }, Modifier.weight(1f), Icons.Default.SettingsBluetooth)
            GhostButton("Keyboard", { onNavigateToScreen("pc_keyboard") }, Modifier.weight(1f), Icons.Default.Computer)
        }
        Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            GhostButton("Media", { onNavigateToScreen("multimedia") }, Modifier.weight(1f), Icons.Default.Movie)
            GhostButton("Presenter", { onNavigateToScreen("presenter") }, Modifier.weight(1f), Icons.Default.PresentToAll)
        }
        Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            GhostButton("Numpad", { onNavigateToScreen("numpad") }, Modifier.weight(1f), Icons.Default.Calculate)
            GhostButton("Gamepad", { onNavigateToScreen("gamepad") }, Modifier.weight(1f), Icons.Default.SportsEsports)
        }
        Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            GhostButton("Pairing", { onNavigateToScreen("scanner") }, Modifier.weight(1f), Icons.Default.QrCodeScanner)
            GhostButton("Help", { onNavigateToScreen("help") }, Modifier.weight(1f), Icons.Default.Help)
        }
    }
}
