...
class SettingsViewModel @Inject constructor(
    ...
    fun setSecureScreen(enabled: Boolean) = viewModelScope.launch { repository.setSecureScreen(enabled) }
    ...
    fun setScrollSpeed(value: Int) = updateMouse { it.copy(scrollSpeed = value.coerceIn(0, 100)) }
    fun setMovementSmoothing(value: Int) = viewModelScope.launch { repository.setMovementSmoothing(value) }
    fun setInvertScroll(enabled: Boolean) = viewModelScope.launch { repository.setInvertScroll(enabled) }
    fun setTapToClick(enabled: Boolean) = updateMouse { it.copy(tapToClick = enabled) }
